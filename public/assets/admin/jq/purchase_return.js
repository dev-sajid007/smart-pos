...
function setCustomer({id, name}) {
    ...
    $('#customer_id').append(selectedCustomer).trigger('change');
}
    ...
    $('.product_name').change(function (e) {
        if (parseFloat(e.target.value)) {
            $.get(`/sales/available_quantity/${e.target.value}`, function (data) {
                ...
                $(e.target).parents('.r-row').find('.unit-cost').val(data.stock_product.product_cost)
...
function changeReturnQuantity(event) {
    ...
    if (quantity != '') {
        ...
        $(event).closest('tr').find('.subtotal').val(amountFormat(parseFloat(quantity) * parseFloat(cost)))
    }
}
