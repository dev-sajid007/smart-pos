...
$(document).ready(function () {
    ...
    // After load default customer
    let customer_id = $('.selected-customer').val()
...
function getCustomerBalance(customer_id) {
    ...
    $.ajax({
        ...
        success: function (res) {
            $('.previous-due').val(res.previous_due.toFixed(2))
            $('.advanced-payment').val(res.advanced_payment.toFixed(2))
...
function calculateTotal(dont_touch_discount) {
    ...
    payable_amount = (invoice_total + invoice_tax - invoice_discount)
    total_payable_amount = (previous_due + payable_amount + currier_amount - advanced_payment)
