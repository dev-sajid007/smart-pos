...
@section('content')
                                                    ...
                                                    {{-- @if (old('product_names') == true)
                                                        @foreach(old('product_names') as $key => $value)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    {{ old('product_names') }}
                                                                    <input type="hidden" name="product_names[{{ $key }}]" class="form-control product-name" value="{{ old('product_names') }}" autocomplete="off">
                                                                    <input type="hidden" name="profit_columns[{{ $key }}]" class="profit-column-input"  value="{{ old('profit_columns') }}">
                                                                    <input type="hidden" name="product_taxes[{{ $key }}]" class="tax-percent"  value="{{ old('productTax') }}">
                                                                    <input type="hidden" name="product_rak_names[{{ $key }}]" class="product-rak-input" value="{{ old('product_rak_names') }}">
                                                                    <input type="hidden" name="has_serials[{{ $key }}]" value="1">
                                                                    ...
                                                                    <p class="item-serial-lists" style="margin-top: 20px !important; margin-bottom: 0;">
                                                                        <input type="hidden" name="product_serials[{{ $key }}][]" value="{{ old('serial') }}">
                                                                        <span class="item-serial badge badge-primary mr-1" style="font-size: 12px">{{ old('serial') }}</span>
                                                                    </p>
                                                                </td>
                                                                <td>
                                                                    <input type="text" name="quantities[{{ $key }}]" readonly class="form-control text-center item-quantity" value="{{ old('quantity') }}">
                                                                </td>
                                                                <td>
                                                                    ...
                                                                    <input type="text" name="unit_prices[{{ $key }}]" class="form-control unit-price changesNo" autocomplete="off" onkeyup="priceEnter(event, this)" value="{{ old('productPrice') }}" onkeypress="return IsNumeric(event);">
                                                                </td>
                                                                <td>
                                                                    <input type="text" name="product_sub_total[{{ $key }}]" class="form-control subtotal" value="{{ old('quantity') * old('productPrice') }}" readonly>
                                                                </td>
                                                                <td>
    ...
    <input type="hidden" class="customer_due_limit" value="{{ optional($customers->where('is_default', 1)->first())->due_limit ?? 999999999999999 }}">
...
<script type="text/javascript">
    ...
    function loadProducts()
    {
        loadDetails({
            ...
            url: '/search-product-by-customer-id/' + ($('.selected-customer').val() | 'dummy') + '/warehouse/' + $('.select-warehouse').val(),
    ...
    {
        $.get('/product/get-product-by-product-serial', { serial : $(object).val() }, function(res) {
            if (res.name !== undefined) {
                ...
                if (serial_count == 0) {
                    ...
                    if($('input[name="' + name + '"]' ).length == 1) {
                        let product_serial = `<span class="item-serial badge badge-primary mr-1" style="font-size: 12px">${ serial }<span class="d-print-none" onMouseOver="this.style.color='red'" onMouseOut="this.style.color='white'" onClick="SerialItem(this)"><i class="fa fa-times-circle" ></i></span></span>`
                        $('input[name="' + name + '"]' ).closest('td').find('.item-serial-lists').append(product_serial)

                        let input_serial = `<input type="hidden" name="product_serials[${productId}][]" value="${serial}">`;                        $('input[name="' + name + '"]' ).closest('td').find('.item-serial-lists').append(input_serial)

                        let totalSerialQty = $('input[name="' + name + '"]' ).closest('td').find('.item-serial').length
                        ...
                        if($('.check-has-additional-item-field').val() == 'yes') {
                            additional_items_field = `<td><input type="text" name="product_item_ids[${ productId }]" value="" class="form-control" placeholder="123"></td>`
                        }
    ...
    {
        if (e.keyCode == 13) {
            if($('.product-discount').is(':visible')){
                $('.product-discount').focus();
            }
            // row_increment()
        } else {
    ...
    function calculateTotal(dont_touch_discount) {
        ...
        $('.subtotal').each(function (index, item) {
            ...
            if (subtotal != '') {
                let tax_percent = ((($(item).closest('tr').find('.tax-percent').val()) * Number(subtotal)) / 100) || 0;
        ...
        // total_payable_amount=(total_payable_amount-receive_amount)(invoice_total_cod*100)/cod_amount;
    ...
    function calculateCODAmount(object)
    {
        ...
        let recieveAmount   = Number($('.receive-amount').val() | 0)

        let codPercent      = Number($(object).val() | 0)

        let pTotal          = invoiceTotal + invoiceTax + currierAmount - discountAmount - recieveAmount;
    ...
    function insertNewItem() {
        ...
        let new_row = `<tr>
                    ...
                    <td>
                        ...
                        <p class="profit-column"></p>
                        <p class="product-rak-text" style="color: #156984; margin-top: -14px; margin-bottom: 0; font-size: 11px"></p>
                        <p class="item-serial-lists" style="margin-top: 20px !important; margin-bottom: 0"  ></p>
                    </td>
                    <td>${ productCode }</td>
                    `
                    + additional_items_field +
