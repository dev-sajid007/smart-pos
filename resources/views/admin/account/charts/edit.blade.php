@extends('admin.master')
@section('title', ' - Chart of account edit')
@section('content')

    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-edit"></i> System Settings</h1>
                <p>Create Company Form</p>
            </div>
            <ul class="app-breadcrumb breadcrumb">
                <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
                <li class="breadcrumb-item">Companies</li>
                <li class="breadcrumb-item"><a href="#">Add Company</a></li>
            </ul>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="tile">
                    <h3 class="tile-title">Update Chart of Account</h3>
                    <div class="tile-body" style="min-height: 440px">
                        <form class="form-horizontal" method="post" action="{{ route('accounts-charts.update', $account_charts->id)}}">
                            @csrf
                            @method('PATCH')

                            <div class="form-group row  add_asterisk">
                                <label class="control-label col-md-3">Gl Account</label>
                                <div class="col-md-8">
                                    <select name="gl_account_id" class="form-control select2" required type="text">
                                        @foreach($gl_accounts as $id => $account)
                                            <option value="{{ $id }}" {{ $id == old('gl_account_id', $account_charts->gl_account_id) ? 'selected' : '' }}>{{ $account }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="form-group row  add_asterisk">
                                <label class="control-label col-md-3">Head Type</label>
                                <div class="col-md-8">
                                    <select name="head_type" class="form-control select2" required type="text">
                                        <option value="{{ $account_charts->head_type }}">
                                        </option>
                                        <option value="0" {{ $account_charts->head_type == 0 ? 'selected' : '' }}>Income</option>
                                        <option value="1" {{ $account_charts->head_type != 0 ? 'selected' : '' }}>Expense</option>
                                    </select>
                                </div>
                            </div>

                            <div class="form-group row add_asterisk">
                                <label class="control-label col-md-3">Head Name</label>
                                <div class="col-md-8">
                                    <input name="head_name" value="{{ $account_charts->head_name }}" class="form-control" type="text" placeholder="Head Name">
                                </div>
                            </div>


                            <div class="tile-footer">
                                <div class="row">
                                    <div class="col-md-12">
                                        <button class="btn btn-primary pull-right" type="submit"><i
                                                    class="fa fa-fw fa-lg fa-check-circle"></i>Update Chart of Account
                                        </button>
                                    </div>
                                </div>
                            </div>
                        </form>
                    </div>

                </div>
            </div>
        </div>
    </main>

    <script src="{{ asset('assets/admin/js/jquery-3.2.1.min.js') }}"></script>
    <script src="{{ asset('assets/admin/js/plugins/select2.min.js')}}"></script>
@endsection
