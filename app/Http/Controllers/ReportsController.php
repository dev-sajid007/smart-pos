...
class ReportsController extends Controller
{
    ...
    public function marketers_report(Request $request){

        if($request->all() != []){

            $marketers = marketer::get();
            $sales     = sale::where('marketers_id', $request->marketers_id)
                          ->when($request->filled('start_date') || $request->filled('end_date'), function ($q) use ($request){
                                $q->whereBetween('sale_date', [$request->start_date, $request->end_date]);
                            })->get();
            return view('admin.reports.marketers_report',compact('marketers', 'sales'));
        }
        else{
            return view('admin.reports.marketers_report',[
                'marketers' => marketer::get(),
                'sales' => [],
            ]);
        }
